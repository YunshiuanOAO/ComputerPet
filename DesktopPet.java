...
public class DesktopPet implements ScreenUsedAlert.AlertCallback {
    ...
    // 番茄鐘應用程式實例 - 使用Map管理每個角色的番茄鐘
    // key: -1 表示主屋，0,1,2... 表示狗狗索引
    private Map<Integer, PomodoroApp> pomodoroApps = new HashMap<>();

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new DesktopPet().initialize());
    }
    ...
    private void initialize() {
        try {
            ...
            gifDog = new ImageIcon("javat/dogmove.gif");
            gifDogDance = new ImageIcon("javat/dogdance.gif");
            gifDogDance2 = new ImageIcon("javat/dogdance2.gif");
        } catch (IOException e) {
    ...
    private void setDogCount(int count) {
        ...
        if (count > dogCount) {
            ...
            for (int i = dogCount - 1; i >= count; i--) {
                // 關閉對應的番茄鐘視窗
                PomodoroApp app = pomodoroApps.get(i);
                if (app != null) {
                    app.dispose();
                    pomodoroApps.remove(i);
                }
    ...
    private JFrame createHomeFrame(BufferedImage img, int x, int y, int showWidth, int showHeight, boolean alwaysOnTop) {
        ...
        item2.addActionListener(e -> {
            // 獲取或創建主屋的番茄鐘
            getOrCreatePomodoroApp(-1, frame.getLocation());
        });
        
        JMenuItem item3 = new JMenuItem("螢幕使用時間提醒");
        item3.addActionListener(e -> {
            String currentTime = getCurrentUsageTime();
            JOptionPane.showMessageDialog(frame, 
                "目前螢幕使用時間: " + currentTime + "\n" +
                "監控狀態: " + (screenAlert.isMonitoring() ? "運行中" : "已停止"),
                "螢幕使用時間", 
        ...
        panel.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (e.isPopupTrigger()) {
                    ...
                    if (transitionTimers.get(0) != null && transitionTimers.get(0).isRunning()) {
                        transitionTimers.get(0).stop();
                    }
                    popupMenu.show(panel, e.getX(), e.getY());
                } else {
                    mouseDownCompCoords[0] = e.getPoint();
                    isDragging = true;
                    if (returnTimer != null && returnTimer.isRunning()) returnTimer.stop();
                    ...
                    dogLabels.get(0).setIcon(new ImageIcon(imgDogCatch.getScaledInstance(showWidth, showHeight, Image.SCALE_SMOOTH)));
                }
            }
            @Override
            public void mouseReleased(MouseEvent e) {
                ...
                if (wasMoving.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(gifDog.getImage().getScaledInstance(showWidth, showHeight, Image.SCALE_DEFAULT)));
                    startRandomMoveAnimation(0);
                } else if (wasTeleporting.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(gifDog.getImage().getScaledInstance(showWidth, showHeight, Image.SCALE_DEFAULT)));
                    startRandomTeleportAnimation(0);
                } else if (wasSitting.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(imgDogSit.getScaledInstance(showWidth, showHeight, Image.SCALE_SMOOTH)));
                } else if (wasCheering.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(imgDogCheer.getScaledInstance(showWidth, showHeight, Image.SCALE_SMOOTH)));
                } else if (wasDancing.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(gifDogDance.getImage().getScaledInstance(showWidth, showHeight, Image.SCALE_DEFAULT)));
                } else if (wasDancing2.get(0)) {
                    dogLabels.get(0).setIcon(new ImageIcon(gifDogDance2.getImage().getScaledInstance(showWidth, showHeight, Image.SCALE_DEFAULT)));
                } else if (isDogFollowingHome && (now.x != homePos.x || now.y != homePos.y)) {
                    dogLabels.get(0).setIcon(new ImageIcon(imgDog.getScaledInstance(showWidth, showHeight, Image.SCALE_SMOOTH)));
                    startTransitionAnimation(
                        now,
    ...
    private JFrame createDogFrame(int showWidth, int showHeight, boolean alwaysOnTop) {
        ...
        item2.addActionListener(e -> {
            // 獲取或創建這隻狗狗的番茄鐘
            getOrCreatePomodoroApp(currentDogIndex, frame.getLocation());
        });
    ...
    public void onAlert() {
        ...
        System.out.println("螢幕使用時間提醒: 視覺提醒開始");
    }
    ...
    public void onAlertEnd() {
        // 結束視覺提醒，恢復正常狀態
        for (int i = 0; i < dogCount; i++) {
            if (!wasMoving.get(i) && !wasTeleporting.get(i) && !wasSitting.get(i) && 
                !wasCheering.get(i) && !wasDancing.get(i) && !wasDancing2.get(i)) {
                dogLabels.get(i).setIcon(new ImageIcon(imgDog.getScaledInstance(dogWidth, dogHeight, Image.SCALE_SMOOTH)));
            }
        }
        System.out.println("螢幕使用時間提醒: 視覺提醒結束");
    }
    ...
    // 為PomodoroApp提供的getter方法
    public JFrame getHomeFrame() {
        return homeFrame;
    }
    
    // 獲取特定狗狗框架的位置
    public Point getDogFrameLocation(int dogIndex) {
        if (dogIndex >= 0 && dogIndex < dogFrames.size()) {
            return dogFrames.get(dogIndex).getLocation();
        }
        return null;
    }
    
    // 檢查特定狗狗框架是否可見
    public boolean isDogFrameVisible(int dogIndex) {
        if (dogIndex >= 0 && dogIndex < dogFrames.size()) {
            return dogFrames.get(dogIndex).isVisible();
        }
        return false;
    }
    
    // 獲取或創建指定角色的番茄鐘
    private PomodoroApp getOrCreatePomodoroApp(int characterIndex, Point location) {
        PomodoroApp app = pomodoroApps.get(characterIndex);
        if (app == null || !app.isDisplayable()) {
            app = new PomodoroApp(this, characterIndex);
            pomodoroApps.put(characterIndex, app);
            
            // 設定初始位置
            Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
            int idealX = location.x - app.getWidth() - 30;
            int idealY = location.y - app.getHeight() - 30;
            
            // 確保位置在螢幕範圍內
            int finalX = Math.max(0, Math.min(idealX, screenSize.width - app.getWidth() - 5));
            int finalY = Math.max(0, Math.min(idealY, screenSize.height - app.getHeight() - 40));
            
            app.setLocation(finalX, finalY);
            app.setVisible(true);
        } else {
            // 如果已經開啟，將其帶到前面並確保可見
            app.toFront();
